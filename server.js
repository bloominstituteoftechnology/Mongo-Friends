...
const BlogPost = require('./Posts/PostModel');
...
server.post('/api/friends', (req, res) => {
  ...
  if ( firstName && lastName && age ) {
    ...
    res
      .status(400)
      .json({ errorMessage: "Please provide the First Name, Last Name and Age for the friend." });
  }
});
      ...
      Friend.findByIdAndUpdate(id, updatedFriend, { new: true })
        .then(friend => {
          if (friend) {
            ...
            res
              .status(404)
              .json({ message: `The friend ID: ${id} could not be found.` });
          }
        })
        .catch(error => {
          if (error.name === 'CastError') {
...
server.post('/api/posts', (req, res) => {
  ...
  if ( author && title && body ) {
    ...
    res
      .status(400)
      .json({ errorMessage: "Please provide Author ID, Title and Body for the post." });
  }
});
    ...
    BlogPost.findByIdAndUpdate(id, updatedPost, { new: true })
      ...
      .catch(error => {
        if (error.name === 'CastError') {
          ...
          res
            .status(500)
            .json({ errorMessage: 'There was an error trying to save that post.' });
        }
      });
  } else {
    res
