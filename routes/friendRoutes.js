...
friendsRouter.post("/", (req, res) => { // create new friend
  ...
  if (age < 1 || age > 120) {
    res
      .status(400)
      .json({ errorMessage: "Age must be an integer between 1 and 120." });
  }
  if (age % 1 !== 0) {
    res.status(400).json({ errorMessage: "Age must be a whole number!" });
  }
